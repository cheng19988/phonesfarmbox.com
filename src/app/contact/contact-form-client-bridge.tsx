    ...
    async function onSubmit(e: Event) {
      ...
      try {
        ...
        if (res.ok && data.ref) {
          ...
          statusEl.innerHTML = `<div class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">${msg} — try <a href="${CONTACT.whatsappUrl}" class="font-semibold underline">WhatsApp</a>.</div>`;
        }
      } catch {
        if (statusEl) {
          statusEl.innerHTML = `<div class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">Network error — try again or use <a href="${CONTACT.whatsappUrl}" class="font-semibold underline">WhatsApp</a>.</div>`;
        }
      } finally {
        if (submitBtn) {
