...
export function ProductTechnicalDataStatus({ slug, data }: Props) {
  ...
  return (
    <section className="mb-12 info-panel">
      <h2 className="text-2xl font-bold text-slate-900 mb-2">Technical data status</h2>
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-6 text-sm">
        <div className="p-3 rounded-lg border border-slate-200 bg-slate-50">
          ...
          <div className="text-slate-800">{imageTypeLabel(data.imageType)}</div>
          ...
          {data.imageLastVerifiedAt && (
            <p className="text-xs text-slate-500 mt-2">Last verified: {data.imageLastVerifiedAt}</p>
          )}
        </div>
        <div className="p-3 rounded-lg border border-slate-200 bg-slate-50">
          <div className="text-xs text-slate-500 uppercase mb-1">Datasheet</div>
          <div className="text-slate-800">{datasheetStatusLabel(data.datasheetStatus)}</div>
        </div>
      </div>
          ...
          {rows.map((row) => {
            ...
            return (
              <tr key={row.key} className="border-b border-slate-200">
                <td className="py-3 text-slate-500 pr-4 w-1/3">{row.label}</td>
                <td className={`py-3 ${isPending ? "text-slate-400 italic" : "text-slate-900"}`}>{display}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <p className="text-sm text-slate-600">
        ...
        <Link href={`/contact?product=${slug}`} className="link-accent">
